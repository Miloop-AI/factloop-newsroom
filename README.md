...
source-backed news article written in that same language. It searches the last week of news,
drafts an article strictly from what it found, verifies its own draft against the sources, and
only then formats the result for search and generative-engine visibility.
...
crawl. Tavily returns exactly that shape — title, matched passage, and a canonical URL per hit —
and exposes a `topic="news"` mode plus a `days` window, so the "last 7 days" rule is a hard API
filter rather than something we post-process and hope for. One dependency covers retrieval and
recency; everything vendor-specific is isolated in `tavily_client.py`.
...
`mcp_server.py`) we hard-wire `topic="news"` and `days=7` into the tool's definition, so no agent
can widen the window or drift off-topic. Mounting a general third-party MCP would re-expose those
knobs and hand the model the ability to relax the very constraint the product depends on. Writing
our own tool also keeps the vendor swappable behind a stable interface.
...
| Researcher | — (pure code, no model) | — |

The Detector is the language classifier from the pipeline's front door; it runs the cheapest, fastest
model because every non-CJK keyword — the common English case included — reaches it.
...
entry point, `api.py`, that drives the **same** `run_newsroom` pipeline — no logic is duplicated.
...
`asyncio.run` internally — invoking it from a running event loop would raise. FastAPI runs sync
...
Keys live only in the server environment — callers never send them. `/generate` is rate-limited per
client IP (default `5/hour`) via slowapi, keyed off the real caller from `X-Forwarded-For` rather
...
not shared across instances — a Redis backend would be the swap-in to scale out.
...
```
src/factloop/
  mcp_server.py      FastMCP server exposing search_news (topic=news, days=7)
